<?php
/**
 * App\src\Controller\Webadmin\LinksController.php
...
class LinksController extends AppController
{
    ...
    private $titleModule = "Link";
    private $primaryModel = "Links";
    ...
    public function add()
    {
        ...
        $listLinksMaps = $this->{$this->primaryModel}->LinksMaps->find('all',[
            'conditions' => [
                'status' => 1
            ],
            'order' => [
        ...
        $linksMaps = $this->Utilities->treeToListSitemaps($listLinksMaps);
        $parents = $this->{$this->primaryModel}->find('treeList',[
            'conditions' => [
                'status' => 1,
                '_type'=>'HEADER'
            ],
        ...
        $contentsLists = $this->{$this->primaryModel}->Contents->ContentsCategories->find('all',[
            'conditions' => [
                'ContentsCategories.status' => 1,
            ],
            'order' => [
                'ContentsCategories.id' => 'ASC',
            ],
            'contain' => [
                'Contents' => function($q){
        ...
        $contents = $this->Utilities->contentsLists($contentsLists);
        $this->set(compact('record','linksMaps','parents','contents'));
    ...
    public function edit($id = null)
    {
        ...
        $contentsLists = $this->{$this->primaryModel}->Contents->ContentsCategories->find('all',[
            'conditions' => [
                'OR' => [
                    'id' => (!empty($record->content) ? $record->content->contents_category_id : '0'),
                    'status' => 1
                ]
            ],
            'order' => [
            ...
            'contain' => [
                'Contents' => function($q) use ($record){
                    return $q->where([
                        'OR' => [
                            'id' => (!empty($record->content) ? $record->content->id : '0'),
                            'status' => 1
                        ]
                    ])
                    ->order(['title'=>'ASC']);
